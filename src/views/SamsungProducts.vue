<!--frontend/src/views/SamsungProducts.vue-->
<template>
    <div class="container-fluid px-5 mt-5">
      <!-- Add your custom component logic here to fetch only Samsung products -->
      ...
      <div v-if="!loading && !products.length">No Samsung products available</div>
      <!-- Pagination -->
  ...
  export default {
    ...
    mounted() {
      this.fetchSamsungProducts();
    },
    methods: {
      async fetchSamsungProducts(page = 1) {
        try {
          const token = localStorage.getItem('token');
          const response = await axios.get(`${API_ROOT_URL}/products?name=Samsung&page=${page}`, {
            headers: {
